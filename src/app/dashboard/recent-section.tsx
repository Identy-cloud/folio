...
export function RecentSection({ presentations, starred, onToggleStar }: Props) {
  ...
  return (
    <section className="mb-6 sm:mb-8">
      <h3 className="mb-3 font-display text-sm tracking-widest text-steel uppercase">
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-neutral-700">
        {recent.map((p) => (
          <div
            key={p.id}
            className="group relative w-56 shrink-0 border border-silver/30 bg-white transition-shadow hover:shadow-lg sm:w-64"
          >
            <Link href={`/editor/${p.id}`} className="block">
              {p.coverSlide ? (
                ...
                <div className="flex aspect-video items-center justify-center bg-[#FAFAFA]">
                  <span className="font-display text-base tracking-tight text-steel sm:text-lg">
                    {p.title}
                  </span>
                </div>
              )}
            ...
            <div className="flex items-center justify-between px-3 py-2">
              <div className="min-w-0 flex-1">
                <p className="truncate text-xs font-medium text-navy">{p.title}</p>
              ...
              <button
                ...
                className={`ml-1 shrink-0 rounded p-1 transition-colors ${
                  starred.has(p.id)
                    ? "text-amber-400"
                    : "text-steel/60 opacity-0 group-hover:opacity-100 hover:text-amber-400"
                }`}

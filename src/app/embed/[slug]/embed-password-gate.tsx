    ...
    <div className="flex h-screen w-screen items-center justify-center bg-black px-4">
      <form onSubmit={handleSubmit} className="w-full max-w-xs space-y-3 text-center">
        ...
        <input
          ...
          autoFocus
          className="w-full border-b border-neutral-700 bg-transparent px-2 py-2.5 text-sm text-white placeholder-neutral-500 outline-none focus:border-white transition-colors"
        />
        {error && <p className="text-xs text-red-400">Incorrect password</p>}
        <button
          type="submit"
          disabled={loading || !pw.trim()}
          className="w-full rounded bg-white py-2.5 text-xs font-semibold tracking-[0.2em] text-black uppercase hover:bg-neutral-200 disabled:opacity-40 transition-colors"
        >
          {loading ? "..." : "Enter"}
        </button>
      </form>

...
export function AnimationTimeline({ onClose }: Props) {
  ...
  return (
    <div className="hidden md:flex flex-col border-t border-steel/30 bg-navy">
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-steel/30">
        <div className="flex items-center gap-2">
          <button
            onClick={handlePlay}
            className="flex h-7 w-7 items-center justify-center rounded bg-white/5 text-silver hover:text-white hover:bg-steel transition-colors"
            aria-label={playing ? "Stop" : "Play animations"}
          >
            <Play size={14} weight={playing ? "fill" : "regular"} />
          </button>
          <span className="text-xs text-silver/70 font-medium">
            Animation Timeline
          </span>
          {animatedElements.length === 0 && (
            <span className="text-xs text-silver/50">— no animated elements</span>
          )}
        </div>
        <button
          onClick={onClose}
          className="flex h-6 w-6 items-center justify-center rounded text-silver/50 hover:text-white hover:bg-steel transition-colors"
      ...
      <div className="relative overflow-x-auto px-3 py-2" style={{ minHeight: "80px" }}>
        <div className="relative" style={{ width: `${TOTAL_WIDTH}px`, minHeight: `${Math.max(animatedElements.length * 36, 36)}px` }}>
          {ticks.map((s) => (
            <div key={s} className="absolute top-0 bottom-0 border-l border-steel/50" style={{ left: `${s * PX_PER_SEC}px` }}>
              <span className="absolute -top-0.5 left-1 text-[9px] text-silver/50">{s}s</span>
            </div>
          ))}
